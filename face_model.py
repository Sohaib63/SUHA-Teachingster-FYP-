...
class FaceModel:
  ...
  def get_input(self, face_img,th):
    r_points=[]
    ...
    #  if bbox[b][4]>=th:
    #    t_bbox.append(bbox[b])
    #bbox=np.asarray(t_bbox)

    bbox = bbox[0:,0:4]
    for pt in range (0,len(points)):
        r_points.append(points[pt,:].reshape((2,5)).T)
    al_images=[]
    for b in range (0,len(bbox)):
        nimg = face_preprocess.preprocess(face_img, bbox[b], r_points[b], image_size='112,112')
        nimg = cv2.cvtColor(nimg, cv2.COLOR_BGR2RGB)
        aligned = np.transpose(nimg, (2,0,1))
        al_images.append(aligned)
    return al_images,bbox
